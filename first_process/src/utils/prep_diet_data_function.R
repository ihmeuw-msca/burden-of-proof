...
prep_diet_data <- function(
  ...
  diet_dir = "/home/j/temp/hkl1/mr_brt/03_evidence_score/input_data/for_ryan/version15",
  verbose = TRUE) {
  
  dev <- FALSE
  if (dev) {
    # ro_pair <- "fruit_ischstroke"
    ro_pair <- "wholegrain_hemstroke"
  ...
  if ("follow_up" %in% names(df)) {
    df <- df %>%
      mutate(
        follow_up_orig = follow_up,
        follow_up = ifelse(follow_up < 10, 1, 0) )
    
    # # commenting out after 06/30/2020 meeting
    # if (any(is.na(df$follow_up_orig))) {
    #   warning("Removing NA values in follow_up variable")
    #   df <- df %>%
    #     filter(!is.na(follow_up_orig))
    # }
  }
  ...
  data_cols = c(
    ...
    'selection_bias', 'reverse_causation', 'total_red', 'beef',
    'pork', 'odds_ratio', 'nonstandard_exp', 'incidence', 'mortality',
    'cvd', 'ischemic', 'hemorrhagic', 'follow_up'
  )
  ...
  for (cov in data_cols[data_cols %in% names(df)]) {
    ...
    # cat(cov, "\n")
    # # commenting this out after June 30, 2020 meeting
    # if (all(is.na(df[, cov]))) next
    # 
    # if (any(is.na(df[, cov]))) {
    #   warning(paste0("Removing NA values from '", cov, "'"))
    #   df <- df %>%
    #     filter(!is.na(!! rlang::sym(cov)))
    # }
    
    if (any(is.na(df[, cov]))) next # 06/30/2020
  ...
  if (all(c("incidence", "mortality") %in% names(df))) {
    ...
    bias_covs <- bias_covs[!bias_covs %in% c("incidence", "mortality")] # added this 07/01/2020
  }
  
  
  # if no total stroke data is present, don't use ischemic/hemorrhagic
  if (all(c("ischemic", "hemorrhagic") %in% names(df))) {
    if (max(df$ischemic + df$hemorrhagic, na.rm = TRUE) - min(df$ischemic + df$hemorrhagic, na.rm = TRUE) == 0) {
      bias_covs <- bias_covs[!bias_covs %in% c("ischemic", "hemorrhagic")]
    }
  } else {
    # remove 'ischemic' and 'hemorrhagic' if 0 or 1 of them are in the dataset
    bias_covs <- bias_covs[!bias_covs %in% c("ischemic", "hemorrhagic")] # added this 07/01/2020
  }
