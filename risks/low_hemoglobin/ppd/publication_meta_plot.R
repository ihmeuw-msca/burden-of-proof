...
library(ggplot2)
library(stringr)
source("PATH TO publication_mr_brt.R")
source("PATH TO publication_plot_code.R")

# load in data sets -------------------------------------------------------

df <- fread("PATHT TO PROCESSED DATASET")
...
fwrite(author_df, 'PATH TO AUTHORS CSV')

bop_df <- fread("PATH TO PROCESSED DATASET FOR MAIN")
# bop_df <- fread("PATH TO PROCESSED DATASET FOR POSTATAL DEPRESSION")
...
bop_forest_plot_w_mrbrt_publication(
  input_df = mrbrt_df,
  main_title = "MR-BRT Analysis - Meta Analyzed Relative Risk - Postnatal Depression",
