...
if(level_100){
  save_dir <- "[path to draws at 100 exposure levels]"
} else {
  save_dir <- "[path to draws at 1000 exposure levels]"
}

pkl_dir <- "[path to mr-brt models]"
...
for (ro in rr_files) {
  print(ro)
  signal_model_path <- paste0(pkl_dir,ro,"/signal_model.pkl")
  linear_model_path <- paste0(pkl_dir,ro,"/new_linear_model.pkl")
  ...
  write.csv(df_re, paste0(save_dir,ro,".csv"), row.names = F)
  ...
  write.csv(df_fe, paste0(save_dir,ro,"_fe.csv"), row.names = F)
  ...
  write.csv(summary_rr, paste0("[path to results folder]",ro,'.csv'), row.names = F)
  ...
  write.csv(summary_rr_fe, paste0("[path to results folder]",ro,'_fe.csv'), row.names = F)
}

# for fractures
fracture_model_path <- paste0(pkl_dir,"smoking_hip_fracture","/model.pkl")
fracture_model <- py_load_object(filename = fracture_model_path, pickle = "dill")

cov_names <- fracture_model$cov_names[!grepl("inter", fracture_model$cov_names)]
...
write.csv(draws, paste0(save_dir,"smoking_fractures",".csv"), row.names = F)
