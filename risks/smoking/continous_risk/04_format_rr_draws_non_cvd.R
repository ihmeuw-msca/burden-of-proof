...
# Set up arguments
if(interactive()){
  ro_pair <- "[enter risk-outcome pair of interest]"
  level_100 <- F
} else {
...
if(level_100){
  ...
  rr_dir <- "[path to raw draws for 100 exposure levels]"
  save_dir <- "[path to final formated draws for 100 exposure levels]"
} else {
  message("1000 exposure levels")
  rr_dir <- "[path to raw draws for 1000 exposure levels]"
  save_dir <- "[path to final formated draws for 1000 exposure levels]"
}
...
if(ro_pair=="fractures"){
  rr_frac <- fread(paste0(rr_dir, "smoking_", ro_pair, ".csv"))
  rr_frac[, rr := exp(rr)]
  ...
  # reshape the data
  rr <- fread(paste0(rr_dir, "smoking_", ro_pair, ".csv"))
...
# save the draws
write.csv(rr_full, paste0(save_dir, ro_pair, ".csv"), row.names = F)
