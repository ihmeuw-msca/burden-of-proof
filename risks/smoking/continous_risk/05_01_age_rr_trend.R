...
age_rr_dt_dir <- '[path to cleaned age-stratified RR data]'
clean_data <- fread(file.path(age_rr_dt_dir, 'all_extractions_cleaned.csv'))
...
write.csv(save_data, file.path(age_rr_dt_dir, "smoking_cvd_age_rr_summary.csv"), row.names = F)
...
write.csv(summary, file.path(age_rr_dt_dir, paste0("attenuation_pct_summary_", ro_pair, "_",age_ref_group,".csv")), row.names = F)

# Save draws of the attenuation factor
write.csv(pct_draws, file.path(age_rr_dt_dir, paste0("attenuation_pct_draws_", ro_pair, "_",age_ref_group,".csv")), row.names = F)
