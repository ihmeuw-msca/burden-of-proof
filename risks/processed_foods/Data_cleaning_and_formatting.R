...
dt$incidence <- ifelse(dt$outcome_type %in% c("Incidence", "Incidence & Mortality", "Incidence, mortality", "Incidence and mortality"), 0, 1)
dt$mortality <- ifelse(dt$outcome_type %in% c("Mortality", "Incidence & Mortality", "Incidence, mortality", "Incidence and mortality"), 1, 0)
...
## Converting the follow_up periods into the same unit
dt$value_of_duration_fup <- as.numeric(dt$value_of_duration_fup)
dt <- dt[, cov_follow_up := ifelse(value_of_duration_fup > 10, 1, 0)]

## Rename the effect size and dose response columns as required by the pipeline
...
names(dt) <- gsub("^(cofounder|confounder|confounders)", "cov", names(dt))

## Drop non-required variables
dt <- dt[, c("cov_other_dietary_components", "cov_other") := NULL]
