...
# 
# Author: Cory Spencer
# Purpose: Clean & prepare GBV RR data for MR-BRT
#
####################################################################################################################################################################
...
#get mapped outcomes
outcome_map <- fread("filepath/combined_outcome_map.csv")
outcome_map <- outcome_map[!is.na(`Covidence_#`)]

ext <- fread(paste0(ext_dir, 'data_download_May152024.csv'))
hdrs <- fread(paste0(ext_dir, 'data_download_Jan19.csv'))
names(ext) <- names(hdrs)

#read in continued extractions
musp <- fread(paste0(ext_dir, 'Continued_Extractions_Mar27_2023_Completed_All_Reviewers_EDIT052024_.csv'), encoding = "UTF-8") 
names(musp)[1:1131] <- names(hdrs)
musp_ids <- unique(musp$`Covidence #`)

#read in long last extraction
...
#intergenerational associations (149506 and 82353) and articles by Chandan et al (#66892, #70567, #65969, and #65029)
excl <- c(149506,82353,66892,70567,65969,65029)
...
all_data[, (var_cols):=lapply(.SD, stri_trans_tolower), .SDcols=var_cols]
...
outcome_map[, `Covidence_#` := as.character(`Covidence_#`)]
all_data <- merge(all_data, 
                  outcome_map[, .(`Covidence_#`, outcome_name, outcome_type, cause, cause_id, cause_or_risk, outcome, outcome_grouping_level4, outcome_grouping_level3, outcome_grouping_level2, done, type, formerly_not_mappable)], 
...
#get counts
notmap <- unique(setdiff(all_data[cause=='not mappable']$`Covidence_#`, all_data[cause!='not mappable']$`Covidence_#`))
na_cause <- unique(setdiff(all_data[is.na(cause)]$`Covidence_#`, all_data[!is.na(cause)]$`Covidence_#`))
length(notmap)
length(na_cause)
length(intersect(notmap, na_cause))
...
data2 <- all_data[, .(`Covidence_#`, Study_ID, Study_name, Title, location_name, ihme_loc_id, Year_start, Year_end,
                      ...
                      outcome_name, outcome_type, outcome_definition, cause, outcome, outcome_grouping_level4, outcome_grouping_level3, outcome_grouping_level2, type, 
...
#manual exclusions of certain information from certain extractions
data2[(Study_ID=='vaeth 2010' & !exposure_definition %like% 'childhood'), exclude:=1]
data3 <- data2[is.na(exclude)]
data3[, exclude:=NULL]

#fix missing study ID
data3[`Covidence_#`==57673, Study_ID:="sorbo 2014"]

all_ids <- unique(data3$`Covidence_#`) 

#create a variable to distinguish new extractions/studies
new_ids <- read.csv(paste0(ext_dir, "new_ids.csv"))
new_ids <- (unique(new_ids$covidence))
data3[ , 'new_study'] = NA
data3 <- mutate(data3, new_study=ifelse(data3$`Covidence_#` %in% new_ids, 1, 0))

write.csv(data3, paste0('filepath/data_cleaned_', gsub('-', '_', Sys.Date()), '.csv'), row.names=F)
